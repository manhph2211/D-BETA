...
class ETSLoss(nn.Module):
    ...
    def _loss(self, image_features, text_features, logit_scale, labels, logit_bias=None):
        logits = self.get_logits(image_features, text_features, logit_scale, logit_bias)
        ground_truth = self.get_ground_truth(labels, image_features.shape[0])
        loss = -F.logsigmoid(ground_truth * logits).sum() / image_features.shape[0]
        return loss

    def forward(self, image_features, text_features, labels, logit_scale=torch.tensor(1.0), logit_bias=None, output_dict=False):

        loss = self._loss(image_features, text_features, logit_scale, labels, logit_bias)
